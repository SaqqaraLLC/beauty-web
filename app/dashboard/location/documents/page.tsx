...
import { LOCATION_REQUIRED_DOCS } from '@/lib/data/stateceRequirements';
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs font-cinzel tracking-[0.06em] ${styles[status] ?? styles.Pending}`}
      style={{ border: '0.5px solid currentColor', opacity: 0.9 }}>
      {status}
    </span>
  );
}
...
export default function LocationDocumentsPage() {
  ...
  useEffect(() => {
    apiGet('/api/documents?ownerType=Location')
      .then(setDocs)
      .catch(() => setDocs([]))
    ...
    <>
      ...
      <div className="min-h-screen bg-saqqara-dark px-4 py-10">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          {/* Header */}
          <div>
            <p className="text-xs font-cinzel tracking-[0.18em] text-saqqara-gold/60 uppercase mb-1">Location Portal</p>
            ...
            <div className="rounded-xl px-4 py-3 flex items-start gap-3"
              ...
              <div>
                ...
                <p className="text-xs text-saqqara-light/50 mt-0.5">
                  {expiringSoon.length} document{expiringSoon.length > 1 ? 's' : ''} expire{expiringSoon.length === 1 ? 's' : ''} within 60 days. Please renew them promptly.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="card" style={{ border: '0.5px solid rgba(201,168,76,0.2)' }}>
            <p className="text-xs font-cinzel tracking-[0.12em] text-saqqara-light/40 uppercase mb-4">Compliance Checklist</p>
            <div className="space-y-2">
              {LOCATION_REQUIRED_DOCS.map(docDef => {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-black/40 text-saqqara-light text-xs px-3 py-2 rounded-lg outline-none"
                  style={{ border: '0.5px solid rgba(201,168,76,0.2)' }}
                  placeholder="e.g. City of Miami Business License"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-cinzel tracking-[0.08em] text-saqqara-light/40 uppercase mb-1.5">
                  License / Permit Number <span className="normal-case text-saqqara-light/20">(optional)</span>
                </label>
                <input
                  type="text"
